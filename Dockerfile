...
RUN sudo pacman -Syu --noconfirm --needed git base-devel && git clone https://aur.archlinux.org/yay.git && cd yay && su nobody -c "makepkg -s --noconfirm" && sudo pacman -U --noconfirm yay-*.pkg.tar.zst && cd .. && rm -rf yay
