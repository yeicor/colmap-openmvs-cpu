...
RUN git clone "https://github.com/KarypisLab/gklib" && cd gklib && sed -i 's/NAMELINK_ONLY//g' CMakeLists.txt && make config cc=gcc prefix=/usr shared=1 && make install && cd .. && rm -rf gklib && \
